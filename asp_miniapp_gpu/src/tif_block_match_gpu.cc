...
int main(int argc, char** argv) {
    ...
    std::cout << disparity_map[idx0] << " DISPARITY_MAP_GPU " << idx0 << " \n";
    std::cout << disparity_map[idx1] << " DISPARITY_MAP_GPU " << idx1 << " \n";
    std::cout << disparity_map[idx2] << " DISPARITY_MAP_GPU " << idx2 << " \n";

    // print out the contents of the disparity_map
    GDALDriver *driver = GetGDALDriverManager()->GetDriverByName("GTiff");
    GDALDataset *disparityDataset = driver->Create("disparitymapgpu.tif", cols, rows, 1, GDT_Float32, nullptr);
    disparityDataset->GetRasterBand(1)->RasterIO(GF_Write, 0, 0, cols, rows, disparity_map.data(), cols, rows, GDT_Float32, 0, 0);
    std::cout << "Output: disparitymapgpu.tif" << " \n";
